...
def header_scan_worker(
    segy_path_or_handle: Union[str, segyio.SegyFile],
...
def write_block_to_segy(
    ...
    ----------
    block_out_path : str
        Path to write the block.
    traces : ndarray
        Trace data.
    headers : ndarray
        Headers for `traces`.
    live : ndarray
        Live mask for `traces`.
    num_samp : int
        Number of samples in traces.
    sample_format : str
        Sample output format. Must be in ("ibm", "ieee").
    endian : str
        Final endianness of the sample format. Must be in ("little", "big").
