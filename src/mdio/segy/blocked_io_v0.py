...
from mdio.core.indexing_v0 import ChunkIterator
from mdio.segy._workers_v0 import trace_worker
...
def _create_executor(num_chunks: int)-> tuple[int, ProcessPoolExecutor]:
    default_cpus = cpu_count(logical=True)
    ...
    return (pool_chunksize, executor)
...
def to_zarr(
    ...
    pool_chunksize, executor = _create_executor(num_chunks=num_chunks)
    with executor:
