...
class TestCreateEmptyMdio:
    ...
    def test_create_empty_like(self, segy_input: Path, empty_mdio_dir: Path) -> None:
        ...
        mdio_unit_aware_template.add_units({"cdp_x": UNITS_METER})
        mdio_unit_aware_template.add_units({"cdp_y": UNITS_METER})

        segy_to_mdio(
            segy_spec=get_teapot_segy_spec(),
            mdio_template=mdio_unit_aware_template,
            input_path=segy_input,
            output_path=mdio_input_tmp,
            overwrite=True,
        )

        ds = create_empty_like(
            input_path=mdio_input_tmp,
            output_path=mdio_output_tmp,
