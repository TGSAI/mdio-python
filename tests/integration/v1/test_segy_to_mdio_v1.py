...
def test_segy_to_mdio_v1__f3() -> None:
    ...
    _validate_variable(ds, "inline", (23,), ["inline"], np.int32, data, True)

    data = np.array([875, 876, 877])
    _validate_variable(ds, "crossline", (18,), ["crossline"], np.int32, data, True)

    data = np.array([0, 4, 8])
    _validate_variable(ds, "time", (75,), ["time"], np.int64, data, True)
    # assert (75,) == dataset["time"].shape

    data = np.array(
        [[6201819, 6201826, 6201833], [6201944, 6201951, 6201958], [6202159, 6202166, 6202173]]
    )
    _validate_variable(ds, "cdp_x", (23, 18), ["inline", "crossline"], np.int32, data, True)
    ...
    _validate_variable(ds, "cdp_y", (23, 18), ["inline", "crossline"], np.int32, data, True)

    data = np.array([[True, True, True], [True, True, True], [True, True, True]])
    _validate_variable(ds, "trace_mask", (23, 18), ["inline", "crossline"], np.bool, data, True)
    ...
    _validate_variable(
        ds, "amplitude", (23, 18, 75), ["inline", "crossline", "time"], np.float32, data, False
    )
