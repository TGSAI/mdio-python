...
def test_make_dataset_metadata_minimal():
    ts: AwareDatetime = datetime.now(timezone.utc)
...
def test_make_toy_dataset():
    ...
    headers_dtype = StructuredType(fields=[
    ...
    ds = make_dataset(
        variables=[
            image, velocity, image_inline, image_headers,
            inline_var, crossline_var, depth_var, cdp_x, cdp_y
        ],
        metadata=meta
    )
    ...
    assert names == [
        "image", "velocity", "image_inline", "image_headers",
        "inline", "crossline", "depth", "cdp-x", "cdp-y"
    ]
